...
class ProductGroceryListViewHolder(view: View) : RecyclerView.ViewHolder(view) {
    ...
    fun render(product: ProductGroceryList, whenProductGroceryListItemSelected: WhenProductGroceryListItemSelected) {
        ...
        if (binding.cbAdquired.isChecked ) {
            binding.tvProduct.paintFlags = binding.tvQuantity.paintFlags or Paint.STRIKE_THRU_TEXT_FLAG
        } else {
            binding.tvProduct.paintFlags = binding.tvQuantity.paintFlags and Paint.STRIKE_THRU_TEXT_FLAG.inv()
        }
